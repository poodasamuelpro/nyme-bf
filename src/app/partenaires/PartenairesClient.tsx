    ...
    <div className="bg-nyme-bg min-h-screen">
      ...
      <section className="section-hero relative overflow-hidden pt-28 sm:pt-32 pb-16 sm:pb-20">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-nyme-orange/20 border border-nyme-orange/30 text-nyme-orange text-sm font-semibold mb-6 font-body">
          ...
          <p className="text-white/70 text-base sm:text-lg leading-relaxed max-w-2xl mx-auto mb-8 font-body">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            ...
            <a
              ...
              className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl bg-white/10 border border-white/25 text-white font-semibold hover:bg-white hover:text-nyme-primary transition-all duration-300 font-body"
            >
              💬 WhatsApp direct
            </a>
          </div>
            ...
            {avantages.map((a) => {
              ...
              return (
                <div key={a.title} className="card p-5 sm:p-6">
                  ...
                  <p className="text-nyme-text-muted text-xs sm:text-sm leading-relaxed font-body">{a.desc}</p>
                </div>
              )
            })}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
            {plans.map((plan) => (
              <div
                key={plan.name}
                className={`relative bg-white rounded-2xl sm:rounded-3xl border-2 ${plan.color} p-6 sm:p-7 flex flex-col shadow-card ${
                  plan.highlight ? 'shadow-nyme-lg scale-[1.02] md:scale-105' : ''
              ...
              >
                ...
                <div className="mb-5">
                  ...
                  <p className="text-nyme-text-muted text-xs sm:text-sm mb-3 font-body">{plan.desc}</p>
                  <div className="flex items-baseline gap-1">
                    <span className={`font-heading text-3xl sm:text-4xl font-black ${plan.highlight ? 'text-nyme-orange' : 'text-nyme-primary'}`}>
                      {plan.price}
                    </span>
                    {plan.period && <span className="text-nyme-text-muted text-xs sm:text-sm font-body">{plan.period}</span>}
                  </div>
                </div>
                <ul className="space-y-2.5 mb-6 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5">
                      ...
                      <span className="text-nyme-text text-xs sm:text-sm font-body">{f}</span>
                    </li>
                  ))}
                </ul>
                <div className="space-y-2">
                  ...
                  <a
                    href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Bonjour NYME, je souhaite m'abonner au plan ${plan.name} (${plan.price} ${plan.period}).`)}`}
                    target="_blank" rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl border border-nyme-border text-nyme-text-muted text-xs font-medium hover:border-nyme-green hover:text-nyme-green transition-all duration-200 font-body"
                  >
      ...
      <section id="contact" className="py-16 sm:py-20 section-light">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            ...
            <a href="mailto:nyme.contact@gmail.com" className="inline-block mt-2 text-nyme-orange text-sm font-semibold hover:underline font-body">
            ...
            <div className="lg:col-span-2 space-y-4">
              {[
                ...
                <div key={item.title} className="card p-4 flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-nyme-text text-sm font-body">{item.title}</div>
              ...
              <a
                href={`https://wa.me/${WHATSAPP_NUMBER}?text=${whatsappMsg}`}
                target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-3 w-full py-4 rounded-2xl bg-nyme-green text-white font-semibold hover:bg-nyme-green/90 transition-all duration-300 shadow-md hover:shadow-lg font-body"
              >
              ...
              <div className="card p-5 sm:p-7">
                {sent ? (
                  <div className="text-center py-10">
                    ...
                    <p className="text-nyme-text-muted text-sm mb-4 font-body">Nous vous recontacterons dans les 4 heures.</p>
                    <a
                      href={`https://wa.me/${WHATSAPP_NUMBER}?text=${whatsappMsg}`}
                      target="_blank" rel="noopener noreferrer"
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5 font-body">Entreprise / Nom *</label>
                      ...
                      <div>
                        <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5 font-body">Votre nom *</label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5 font-body">Email *</label>
                        <input type="email" required value={form.email} onChange={e => setForm({...form, email: e.target.value})}
                          placeholder="vous@entreprise.com" className="input-nyme" />
                      </div>
                      <div>
                        <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5 font-body">Téléphone / WhatsApp *</label>
                    ...
                    <div>
                      <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5 font-body">Plan souhaité</label>
                    ...
                    <div>
                      <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5 font-body">Volume mensuel & besoins</label>
                    ...
                    {error && (
                      <div className="p-3 rounded-xl bg-nyme-red/10 border border-nyme-red/20 text-nyme-red text-xs font-body">⚠️ {error}</div>
                    )}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <a
                        href={`https://wa.me/${WHATSAPP_NUMBER}?text=${whatsappMsg}`}
                        target="_blank" rel="noopener noreferrer"
                        className="flex items-center justify-center gap-2 py-3.5 rounded-xl bg-nyme-green/10 border border-nyme-green/20 text-nyme-green font-semibold text-sm hover:bg-nyme-green hover:text-white transition-all duration-300 font-body"
                      >
                        💬 WhatsApp
                      </a>
                    </div>
