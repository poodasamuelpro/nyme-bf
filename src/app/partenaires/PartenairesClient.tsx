...
import { CheckCircle, ArrowRight, Star, Shield, Clock, Zap, Send, Package, TrendingUp, Users } from 'lucide-react'

const WHATSAPP_NUMBER = '22600000000' // ← Remplace par ton vrai numéro
...
const plans = [
  ...
  },
  {
    ...
    color: 'border-nyme-blue',
    badge: '⭐ Populaire',
    badgeColor: 'bg-nyme-orange text-white',
    highlight: true,
    features: [
  ...
  },
  {
    ...
    color: 'border-nyme-violet',
    badge: '🏢 Premium',
    badgeColor: 'bg-nyme-violet text-white',
    features: [
...
const avantages = [
  { icon: Package, title: 'Livreur dédié', desc: 'Un coursier assigné uniquement à votre compte. Il connaît vos habitudes et vos adresses fréquentes.', color: 'text-nyme-blue', bg: 'bg-nyme-blue/10' },
  ...
  { icon: Clock, title: 'Disponibilité garantie', desc: 'Votre livreur est disponible pendant vos horaires d\'activité définis à l\'avance.', color: 'text-nyme-blue', bg: 'bg-nyme-blue/10' },
]
    ...
    <div className="bg-nyme-bg min-h-screen">
      ...
      <section className="section-hero relative overflow-hidden pt-28 sm:pt-32 pb-16 sm:pb-20">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-nyme-orange/20 border border-nyme-orange/30 text-nyme-orange text-sm font-semibold mb-6">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            ...
            <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${whatsappMsg}`} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl bg-white/10 border border-white/25 text-white font-semibold hover:bg-white hover:text-nyme-blue transition-all duration-300">
              💬 WhatsApp direct
            </a>
          </div>
        </div>
      ...
      <section className="py-16 sm:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-14">
            ...
            <p className="text-nyme-text-muted text-base sm:text-lg max-w-xl mx-auto">
            ...
            {avantages.map((a, i) => {
              ...
              return (
                <div key={a.title} className="card p-5 sm:p-6">
                  ...
                  <p className="text-nyme-text-muted text-xs sm:text-sm leading-relaxed">{a.desc}</p>
                </div>
              )
            })}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
            {plans.map((plan) => (
              <div key={plan.name}
                ...
                {plan.badge && (
                  <div className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-xs font-bold ${plan.badgeColor} shadow-sm`}>
                    {plan.badge}
                  </div>
                )}
                <div className="mb-5">
                  <h3 className="font-heading text-xl font-black text-nyme-text mb-1">{plan.name}</h3>
                  <p className="text-nyme-text-muted text-xs sm:text-sm mb-3">{plan.desc}</p>
                  <div className="flex items-baseline gap-1">
                    <span className={`font-heading text-3xl sm:text-4xl font-black ${plan.highlight ? 'text-nyme-orange' : 'text-nyme-blue'}`}>{plan.price}</span>
                    {plan.period && <span className="text-nyme-text-muted text-xs sm:text-sm">{plan.period}</span>}
                  </div>
                </div>
                <ul className="space-y-2.5 mb-6 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5">
                      ...
                      <span className="text-nyme-text text-xs sm:text-sm">{f}</span>
                    </li>
                  ))}
                </ul>
                <div className="space-y-2">
                  <a href="#contact"
                    className={`flex items-center justify-center gap-2 w-full py-3 sm:py-3.5 rounded-xl font-semibold text-sm transition-all duration-300 ${plan.ctaStyle}`}>
                  ...
                  <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Bonjour NYME, je souhaite m'abonner au plan ${plan.name} (${plan.price} ${plan.period}).`)}`}
                    ...
                    className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl border border-nyme-border text-nyme-text-muted text-xs font-medium hover:border-nyme-green hover:text-nyme-green transition-all duration-200">
      ...
      <section id="livreur-dedié" className="py-16 sm:py-20 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
            {[
              { n:'01', emoji:'📋', title:'Choisissez votre plan', desc:'Sélectionnez la formule adaptée à votre volume mensuel de livraisons.' },
              { n:'02', emoji:'🤝', title:'On vous assigne un livreur', desc:'Un coursier vérifié est dédié à votre compte dès le début du mois.' },
              { n:'03', emoji:'📦', title:'Commandez à tout moment', desc:'Via l\'app, WhatsApp ou votre dashboard privé. Le livreur arrive en express.' },
              { n:'04', emoji:'📊', title:'Suivez tout en temps réel', desc:'Photos, suivi GPS, historique complet et rapports mensuels téléchargeables.' },
            ].map((s, i) => (
              <div key={s.n} className="card p-4 sm:p-5 text-center">
                <div className="relative inline-flex mb-3">
                  ...
                  <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-nyme-blue text-white text-[10px] font-bold flex items-center justify-center">
                    {i + 1}
                  </div>
                </div>
                <h3 className="font-heading text-sm font-bold text-nyme-text mb-1.5">{s.title}</h3>
                <p className="text-nyme-text-muted text-xs leading-relaxed">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="contact" className="py-16 sm:py-20 section-light">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            ...
            <p className="text-nyme-text-muted text-base sm:text-lg max-w-xl mx-auto">
              Remplissez ce formulaire ou contactez-nous directement sur WhatsApp. On vous répond sous 4h.
            </p>
          </div>
            ...
            <div className="lg:col-span-2 space-y-4">
              {[
                ...
                <div key={item.title} className="card p-4 flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-nyme-text text-sm">{item.title}</div>
                    <div className="text-nyme-text-muted text-xs">{item.sub}</div>
                  </div>
                </div>
              ))}
              ...
              <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${whatsappMsg}`}
                target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-3 w-full py-4 rounded-2xl bg-nyme-green text-white font-semibold hover:bg-nyme-green/90 transition-all duration-300 shadow-md hover:shadow-lg">
              ...
              <div className="card p-5 sm:p-7">
                {sent ? (
                  <div className="text-center py-10">
                    ...
                    <p className="text-nyme-text-muted text-sm mb-4">Nous vous recontacterons dans les 4 heures.</p>
                    <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${whatsappMsg}`} target="_blank" rel="noopener noreferrer"
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5">Entreprise / Nom *</label>
                      ...
                      <div>
                        <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5">Votre nom *</label>
                        <input type="text" required value={form.nom} onChange={e => setForm({...form, nom: e.target.value})}
                          placeholder="Prénom Nom" className="input-nyme" />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="email" required value={form.email} onChange={e => setForm({...form, email: e.target.value})}
                          placeholder="vous@entreprise.com" className="input-nyme" />
                      </div>
                      <div>
                        <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5">Téléphone / WhatsApp *</label>
                        <input type="tel" required value={form.telephone} onChange={e => setForm({...form, telephone: e.target.value})}
                          placeholder="+226 XX XX XX XX" className="input-nyme" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5">Plan souhaité</label>
                    ...
                    <div>
                      <label className="block text-nyme-text-muted text-xs uppercase tracking-wider font-medium mb-1.5">Volume mensuel & besoins</label>
                      <textarea rows={4} value={form.message} onChange={e => setForm({...form, message: e.target.value})}
                        placeholder="Ex: J'envoie environ 50 colis/mois depuis ma boutique à Ouaga 2000..."
                        className="input-nyme resize-none" />
                    </div>
                    {error && (
                      <div className="p-3 rounded-xl bg-nyme-red/10 border border-nyme-red/20 text-nyme-red text-xs">⚠️ {error}</div>
                    )}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${whatsappMsg}`} target="_blank" rel="noopener noreferrer"
                        className="flex items-center justify-center gap-2 py-3.5 rounded-xl bg-nyme-green/10 border border-nyme-green/20 text-nyme-green font-semibold text-sm hover:bg-nyme-green hover:text-white transition-all duration-300">
                        💬 WhatsApp
                      </a>
                    </div>
                  </form>
