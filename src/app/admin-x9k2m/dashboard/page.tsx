'use client' 
import { useEffect, useState, useCallback, useMemo } from 'react' import { useRouter } from 'next/navigation' import { supabase } from '@/lib/supabase' import { Zap, LogOut, Users, Package, TrendingUp, ShieldCheck, Plus, X, AlertCircle, CheckCircle, RefreshCw, Eye, Building2, User, Phone, Mail, Loader2, BarChart3, Wallet, FileCheck, Search, Ban, UserCheck, Send, DollarSign, CreditCard, ArrowDownLeft, ChevronDown, ChevronUp, Truck, Clock, XCircle, CheckCircle2, AlertTriangle, Ruler, Calculator, PlusCircle, Trash2, Info } from 'lucide-react' import Link from 'next/link'
// ── Composant Tarification Panel (barème + graphique prix/km + simulateur) ── interface TarificationPanelProps { baremes: BaremeAdmin[] configTarif: ConfigAdmin | null loadingTarifs: boolean savingTarif: string | null editBareme: BaremeAdmin | null setEditBareme: (b: BaremeAdmin | null) => void saveBareme: (b: BaremeAdmin) => void saveConfig: () => void setConfigTarif: (c: ConfigAdmin) => void loadTarifs: () => void token?: string }
interface BaremeAdmin { id: string; km_min: number; km_max: number; prix_par_km: number; label: string; ordre: number; actif: boolean } interface ConfigAdmin { id: string; frais_fixe_immediate: number; frais_fixe_urgente: number; frais_fixe_programmee: number prix_minimum: number; multiplicateur_urgente: number; multiplicateur_programmee: number multiplicateur_pluie: number; pluie_actif: boolean commission_immediate: number; commission_urgente: number; commission_programmee: number tarif_km: number; tarif_minute: number; frais_fixe: number; commission_pct: number; actif: boolean }
// Prix simulé pour un trajet donné function simulerPrix( distanceKm: number, type: 'immediate' | 'urgente' | 'programmee', baremes: BaremeAdmin[], config: ConfigAdmin, pluie = false ): number { let prix = type === 'urgente' ? (config.frais_fixe_urgente || 800) : type === 'programmee' ? (config.frais_fixe_programmee || 800) : (config.frais_fixe_immediate || 800)
let kmRestant = distanceKm const sorted = [...baremes].filter(b => b.actif).sort((a, b) => a.ordre - b.ordre)
for (const b of sorted) { if (kmRestant <= 0) break const km = Math.min(kmRestant, b.km_max - b.km_min) prix += km * b.prix_par_km kmRestant -= km } if (kmRestant > 0 && sorted.length > 0) { prix += kmRestant * sorted[sorted.length - 1].prix_par_km }
if (type === 'urgente') prix *= (config.multiplicateur_urgente || 1.25) if (type === 'programmee') prix *= (config.multiplicateur_programmee || 0.90) if (pluie && config.pluie_actif) prix *= (config.multiplicateur_pluie || 1.15)
return Math.max(config.prix_minimum || 800, Math.round(prix)) }
const CONFIG_LABELS: Record<string, { label: string; unit: string; desc: string }> = { frais_fixe_immediate: { label: 'Frais fixes immédiat', unit: 'XOF', desc: 'Base de calcul pour une course immédiate' }, frais_fixe_urgente: { label: 'Frais fixes urgente', unit: 'XOF', desc: 'Base pour course urgente (avant multiplicateur)' }, frais_fixe_programmee: { label: 'Frais fixes programmée', unit: 'XOF', desc: 'Base pour course programmée (avant multiplicateur)' }, prix_minimum: { label: 'Prix minimum', unit: 'XOF', desc: 'Prix plancher absolu quel que soit le trajet' }, multiplicateur_urgente: { label: 'Multiplicateur urgente', unit: '×', desc: 'Coefficient appliqué aux courses urgentes (ex: 1.25 = +25%)' }, multiplicateur_programmee:{ label: 'Multiplicateur programmée', unit: '×', desc: 'Coefficient pour courses programmées (ex: 0.90 = -10%)' }, multiplicateur_pluie: { label: 'Multiplicateur pluie', unit: '×', desc: 'Coefficient météo si pluie activée' }, commission_immediate: { label: 'Commission NYME immédiat', unit: 'XOF', desc: 'Commission fixe NYME pour course immédiate' }, commission_urgente: { label: 'Commission NYME urgente', unit: 'XOF', desc: 'Commission fixe NYME pour course urgente' }, commission_programmee: { label: 'Commission NYME programmée',unit: 'XOF', desc: 'Commission fixe NYME pour course programmée' }, pluie_actif: { label: 'Météo activée', unit: '', desc: 'Activer le surcoût lors de la pluie' }, }
function TarificationPanel({ baremes, configTarif, loadingTarifs, savingTarif, editBareme, setEditBareme, saveBareme, saveConfig, setConfigTarif, loadTarifs, token, }: TarificationPanelProps) {
const [simDist, setSimDist] = useState(5) const [simType, setSimType] = useState<'immediate' | 'urgente' | 'programmee'>('immediate') const [simPluie, setSimPluie] = useState(false) const [addMode, setAddMode] = useState(false) const [newBareme, setNewBareme] = useState({ km_min: 0, km_max: 0, prix_par_km: 0, label: '', ordre: 0 }) const [saving, setSaving] = useState(false)
// Graphique prix simulés pour distances 0 à 25km const chartPoints = useMemo(() => { if (!configTarif || baremes.length === 0) return [] return Array.from({ length: 26 }, (_, i) => ({ km: i, immediate: simulerPrix(i, 'immediate', baremes, configTarif), urgente: simulerPrix(i, 'urgente', baremes, configTarif), programmee: simulerPrix(i, 'programmee', baremes, configTarif), })) }, [baremes, configTarif])
const maxPrice = useMemo(() => Math.max(...chartPoints.map(p => p.urgente), 1), [chartPoints])
const simResult = useMemo(() => { if (!configTarif || baremes.length === 0) return null const immediate = simulerPrix(simDist, 'immediate', baremes, configTarif, simPluie) const urgente = simulerPrix(simDist, 'urgente', baremes, configTarif, simPluie) const programmee = simulerPrix(simDist, 'programmee', baremes, configTarif, simPluie) const commission = simType === 'urgente' ? configTarif.commission_urgente : simType === 'programmee' ? configTarif.commission_programmee : configTarif.commission_immediate const prixFinal = simType === 'immediate' ? immediate : simType === 'urgente' ? urgente : programmee return { immediate, urgente, programmee, prixFinal, gainCoursier: prixFinal - commission, commission } }, [simDist, simType, simPluie, baremes, configTarif])
const handleAddBareme = async () => { if (!token) return setSaving(true) try { const res = await fetch('/api/admin/tarifs', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': Bearer ${token} }, body: JSON.stringify(newBareme), }) if (res.ok) { setAddMode(false) setNewBareme({ km_min: 0, km_max: 0, prix_par_km: 0, label: '', ordre: 0 }) loadTarifs() } } finally { setSaving(false) } }
if (loadingTarifs) { return ( 
) }
return ( 
Tarification
Actualiser 
  ...
  <div className="bg-gradient-to-br from-[#0A2E8A] to-[#1A4FBF] rounded-2xl p-6 text-white">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
      <div>
        ...
        <input type="number" min="0.5" max="50" step="0.5" value={simDist}
      ...
      <div>
        ...
        <select value={simType} onChange={e => setSimType(e.target.value as 'immediate' | 'urgente' | 'programmee')}
          className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-xl text-white text-sm font-bold focus:outline-none focus:border-[#E87722]">
      ...
      <div className="flex items-end">
        <label className="flex items-center gap-2 cursor-pointer">
          <div className={`w-10 h-5 rounded-full transition-colors ${simPluie ? 'bg-[#E87722]' : 'bg-white/20'} relative`}>
            <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${simPluie ? 'translate-x-5' : ''}`}/>
          </div>
          <span className="text-white/80 text-xs" onClick={() => setSimPluie(!simPluie)}>🌧️ Pluie</span>
        </label>
      </div>
    </div>
    {simResult && (
    ...
    {addMode && (
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          ...
          <div key={f.key}>
            ...
            <input
              ...
              onChange={e => setNewBareme({ ...newBareme, [f.key]: f.type === 'number' ? parseFloat(e.target.value) || 0 : e.target.value })}
        ...
        <div className="col-span-full flex gap-2">
          <button onClick={handleAddBareme} disabled={saving}
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left">
        ...
        <tbody className="divide-y divide-slate-50">
          {[...baremes].sort((a, b) => a.ordre - b.ordre).map(b => (
            <>
              <tr key={b.id} className="hover:bg-slate-50/50">
                ...
                <td className="px-4 py-3">
                  <button onClick={() => setEditBareme(editBareme?.id === b.id ? null : { ...b })}
              ...
              {editBareme?.id === b.id && (
                <tr key={`edit-${b.id}`}>
                  <td colSpan={6} className="px-4 py-4 bg-blue-50 border-b border-blue-100">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                      {[
                        ...
                        <div key={f.key}>
                          ...
                          <input type="number" step={f.key === 'prix_par_km' ? '10' : '0.5'}
                      ...
                      <div>
                        ...
                        <input type="text" value={editBareme.label}
                      ...
                      <div className="flex items-end">
                        <label className="flex items-center gap-2 cursor-pointer pb-2">
                          <input type="checkbox" checked={editBareme.actif}
                    ...
                    <button onClick={() => saveBareme(editBareme)} disabled={savingTarif === b.id}
          ...
          {baremes.length === 0 && (
            <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-400 text-sm">Aucun barème configuré</td></tr>
          )}
        </tbody>
      </table>
    </div>
  ...
  {configTarif && (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {Object.entries(CONFIG_LABELS).filter(([key]) => key in configTarif).map(([key, meta]) => (
          <div key={key} className="bg-slate-50 rounded-xl p-3">
            <div className="flex items-center gap-1.5 mb-1">
              <label className="text-xs font-bold text-slate-600">{meta.label}</label>
              <span title={meta.desc} className="cursor-help"><Info size={10} className="text-slate-400"/></span>
            </div>
            ...
            {key === 'pluie_actif' ? (
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox"
                  ...
                  className="w-4 h-4"/>
                <span className="text-sm font-bold text-slate-700">{configTarif.pluie_actif ? 'Activé 🌧️' : 'Désactivé'}</span>
              </label>
            ) : (
      ...
      <button onClick={saveConfig} disabled={savingTarif === 'config'}
...
// ── Types ──────────────────────────────────────────────────────────────────── interface PartenaireAdmin { id: string; user_id: string; entreprise: string; nom_contact: string telephone: string | null; email_pro: string | null plan: 'starter' | 'business' | 'enterprise' statut: 'actif' | 'suspendu' | 'en_attente' | 'rejete' livraisons_max: number; livraisons_mois: number taux_commission: number; date_debut: string; created_at: string }
interface CoursierAdmin { id: string; nom: string; email: string; telephone: string statut_verification: 'en_attente' | 'verifie' | 'rejete' statut: 'hors_ligne' | 'disponible' | 'occupe' cni_recto_url: string; cni_verso_url: string; permis_url: string total_gains: number; total_courses: number; created_at: string wallet_solde?: number }
interface ClientAdmin { id: string; nom: string; email: string; telephone: string est_actif: boolean; est_verifie: boolean created_at: string; total_livraisons?: number }
interface LivraisonAdmin { id: string; client_nom: string; coursier_nom: string statut: string; type: string; depart_adresse: string arrivee_adresse: string; prix_final: number; created_at: string }
interface WalletAdmin { id: string; user_id: string; solde: number; total_gains: number total_retraits: number; created_at: string utilisateur?: { nom: string; email: string; role: string } }
interface Transaction { id: string; wallet_id: string; type: string; montant: number statut: string; description: string; created_at: string }
// ── Configs ────────────────────────────────────────────────────────────────── const PLAN_CFG = { starter: { label:'Starter', color:'text-green-600 bg-green-50 border-green-200' }, business: { label:'Business', color:'text-orange-600 bg-orange-50 border-orange-200' }, enterprise: { label:'Enterprise', color:'text-purple-600 bg-purple-50 border-purple-200' }, }
const STATUT_CFG: Record<string, { label: string; color: string; dot: string }> = { actif: { label:'Actif', color:'text-green-600 bg-green-50 border-green-200', dot:'bg-green-500' }, en_attente: { label:'En attente', color:'text-amber-600 bg-amber-50 border-amber-200', dot:'bg-amber-400' }, suspendu: { label:'Suspendu', color:'text-red-600 bg-red-50 border-red-200', dot:'bg-red-500' }, rejete: { label:'Rejeté', color:'text-gray-600 bg-gray-50 border-gray-200', dot:'bg-gray-400' }, verifie: { label:'Vérifié', color:'text-green-600 bg-green-50 border-green-200', dot:'bg-green-500' }, hors_ligne: { label:'Hors ligne', color:'text-gray-600 bg-gray-50 border-gray-200', dot:'bg-gray-400' }, disponible: { label:'Disponible', color:'text-green-600 bg-green-50 border-green-200', dot:'bg-green-500' }, occupe: { label:'Occupé', color:'text-blue-600 bg-blue-50 border-blue-200', dot:'bg-blue-500' }, }
const ONGLETS = [ { id:'overview', label:'Vue générale', icon:BarChart3 }, { id:'partenaires', label:'Partenaires', icon:Building2 }, { id:'coursiers', label:'Coursiers', icon:Truck }, { id:'clients', label:'Clients', icon:Users }, { id:'livraisons', label:'Courses', icon:Package }, { id:'wallet', label:'Wallet/Finances', icon:Wallet }, { id:'tarification', label:'Tarification', icon:TrendingUp }, { id:'creation', label:'Actions Admin', icon:Plus }, ]
function Badge({ statut }: { statut: string }) { const cfg = STATUT_CFG[statut] || STATUT_CFG.en_attente return ( <span className={inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border ${cfg.color}}> <span className={w-1.5 h-1.5 rounded-full ${cfg.dot}}/> {cfg.label} ) }
// ── Composant principal ─────────────────────────────────────────────────────── export default function AdminDashboard() { const router = useRouter() const [adminUser, setAdminUser] = useState(null) const [partenaires, setPartenaires] = useState<PartenaireAdmin[]>([]) const [coursiers, setCoursiers] = useState<CoursierAdmin[]>([]) const [clients, setClients] = useState<ClientAdmin[]>([]) const [livraisons, setLivraisons] = useState<LivraisonAdmin[]>([]) const [wallets, setWallets] = useState<WalletAdmin[]>([]) const [loading, setLoading] = useState(true) const [refreshing, setRefreshing] = useState(false) const [onglet, setOnglet] = useState('overview') const [recherche, setRecherche] = useState('') const [error, setError] = useState('') const [success, setSuccess] = useState('')
const [formPartenaire, setFormPartenaire] = useState({ entreprise:'', nom_contact:'', email:'', telephone:'', plan:'starter', adresse:'' }) const [formAdmin, setFormAdmin] = useState({ email:'', nom:'' }) const [creating, setCreating] = useState(false)
const [modalPaiement, setModalPaiement] = useState<{ coursier: CoursierAdmin | null; montant: string; description: string }>({ coursier: null, montant: '', description: '' })
const [openDoc, setOpenDoc] = useState<string | null>(null)
const [baremes, setBaremes] = useState<any[]>([]) const [configTarif, setConfigTarif] = useState(null) const [loadingTarifs, setLoadingTarifs] = useState(false) const [savingTarif, setSavingTarif] = useState<string | null>(null) const [editBareme, setEditBareme] = useState(null)
const stats = { partenaires_total: partenaires.length, partenaires_actifs: partenaires.filter(p => p.statut === 'actif').length, partenaires_attente: partenaires.filter(p => p.statut === 'en_attente').length, coursiers_total: coursiers.length, coursiers_verifies: coursiers.filter(c => c.statut_verification === 'verifie').length, coursiers_attente: coursiers.filter(c => c.statut_verification === 'en_attente').length, clients_total: clients.length, clients_actifs: clients.filter(c => c.est_actif).length, livraisons_total: livraisons.length, ca_total: livraisons.reduce((acc, l) => acc + (l.prix_final || 0), 0), wallets_total_solde: wallets.reduce((acc, w) => acc + (w.solde || 0), 0), }
// ── Auth & Chargement ──────────────────────────────────────────────────────── useEffect(() => { supabase.auth.getSession().then(async ({ data: { session } }) => { if (!session) { router.replace('/admin-x9k2m/login'); return } const { data: u } = await supabase .from('utilisateurs').select('role,nom').eq('id', session.user.id).single() if (!u || u.role !== 'admin') { await supabase.auth.signOut(); router.replace('/admin-x9k2m/login'); return } setAdminUser({ ...session.user, nom: u.nom }) loadData() }) }, [router])
const loadData = useCallback(async () => { setRefreshing(true) try { const [partsRes, coursRes, clientsRes, livsRes, walletsRes] = await Promise.all([ supabase.from('partenaires').select('').order('created_at', { ascending: false }), supabase.from('coursiers') .select(', utilisateurs(nom, email, telephone)') .order('created_at', { ascending: false }), supabase.from('utilisateurs') .select('id, nom, email, telephone, est_actif, est_verifie, created_at') .eq('role', 'client') .order('created_at', { ascending: false }) .limit(100), supabase.from('livraisons') .select(', client:utilisateurs!livraisons_client_id_fkey(nom), coursier:utilisateurs!livraisons_coursier_id_fkey(nom)') .order('created_at', { ascending: false }) .limit(100), supabase.from('wallets') .select(', utilisateurs(nom, email, role)') .order('solde', { ascending: false }) .limit(100), ])
...
// ── Tarification ───────────────────────────────────────────────────────────── const loadTarifs = useCallback(async () => { setLoadingTarifs(true) try { const session = await supabase.auth.getSession() const token = session.data.session?.access_token const res = await fetch('/api/admin/tarifs', { headers: { 'Authorization': Bearer ${token} }, }) const data = await res.json() if (data.baremes) setBaremes(data.baremes) if (data.config) setConfigTarif(data.config) } catch (err: any) { setError('Erreur chargement tarifs: ' + err.message) } finally { setLoadingTarifs(false) } }, [])
const saveBareme = async (bareme: any) => { setSavingTarif(bareme.id) try { const session = await supabase.auth.getSession() const token = session.data.session?.access_token const res = await fetch('/api/admin/tarifs', { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': Bearer ${token} }, body: JSON.stringify({ type: 'bareme', id: bareme.id, data: bareme }), }) const data = await res.json() if (!res.ok) throw new Error(data.error) setSuccess(Barème "${bareme.label}" mis à jour) setEditBareme(null) loadTarifs() } catch (err: any) { setError(err.message) } finally { setSavingTarif(null) } }
const saveConfig = async () => { if (!configTarif) return setSavingTarif('config') try { const session = await supabase.auth.getSession() const token = session.data.session?.access_token const res = await fetch('/api/admin/tarifs', { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': Bearer ${token} }, body: JSON.stringify({ type: 'config', data: configTarif }), }) const data = await res.json() if (!res.ok) throw new Error(data.error) setSuccess('Configuration tarifaire mise à jour') } catch (err: any) { setError(err.message) } finally { setSavingTarif(null) } }
useEffect(() => { if (onglet === 'tarification' && baremes.length === 0) { loadTarifs() } }, [onglet, loadTarifs])
// ── Actions partenaires ────────────────────────────────────────────────────── const updateStatutPartenaire = async (id: string, statut: string) => { setError(''); setSuccess('') const { error: err } = await supabase .from('partenaires').update({ statut, updated_at: new Date().toISOString() }).eq('id', id) if (err) setError(err.message) else { setSuccess(Statut partenaire mis à jour : ${statut}); loadData() } }
// ── Actions coursiers ──────────────────────────────────────────────────────── const validerCoursier = async (id: string, statut: string) => { setError(''); setSuccess('') const { error: err } = await supabase .from('coursiers').update({ statut_verification: statut }).eq('id', id) if (err) setError(err.message) else { setSuccess(Coursier ${statut === 'verifie' ? 'vérifié' : 'rejeté'} avec succès); loadData() } }
// ── Paiement coursier ──────────────────────────────────────────────────────── const payerCoursier = async () => { if (!modalPaiement.coursier || !modalPaiement.montant) return setCreating(true); setError('') try { const session = await supabase.auth.getSession() const token = session.data.session?.access_token const res = await fetch('/api/admin/payer-coursier', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': Bearer ${token} }, body: JSON.stringify({ coursier_id: modalPaiement.coursier.id, montant: parseFloat(modalPaiement.montant), description: modalPaiement.description || Paiement admin — ${new Date().toLocaleDateString('fr-FR')}, }), }) const data = await res.json() if (!res.ok) throw new Error(data.error || 'Erreur paiement') setSuccess(✅ Paiement de ${parseFloat(modalPaiement.montant).toLocaleString()} FCFA effectué pour ${modalPaiement.coursier.nom}) setModalPaiement({ coursier: null, montant: '', description: '' }) loadData() } catch (err: any) { setError(err.message) } finally { setCreating(false) } }
// ── Actions clients ────────────────────────────────────────────────────────── const toggleClientActif = async (id: string, actif: boolean) => { setError(''); setSuccess('') const { error: err } = await supabase .from('utilisateurs').update({ est_actif: actif, updated_at: new Date().toISOString() }).eq('id', id) if (err) setError(err.message) else { setSuccess(Client ${actif ? 'activé' : 'désactivé'}); loadData() } }
// ── Création partenaire ────────────────────────────────────────────────────── const handleCreatePartenaire = async (e: React.FormEvent) => { e.preventDefault(); setCreating(true); setError(''); setSuccess('') try { const session = await supabase.auth.getSession() const token = session.data.session?.access_token const res = await fetch('/api/admin/create-partenaire', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': Bearer ${token} }, body: JSON.stringify(formPartenaire), }) const data = await res.json() if (!res.ok) throw new Error(data.error || 'Erreur création') setSuccess(✅ Partenaire créé et email envoyé !) setFormPartenaire({ entreprise:'', nom_contact:'', email:'', telephone:'', plan:'starter', adresse:'' }) loadData() } catch (err: any) { setError(err.message) } finally { setCreating(false) } }
// ── Promotion admin ────────────────────────────────────────────────────────── const handlePromoteAdmin = async (e: React.FormEvent) => { e.preventDefault(); setCreating(true); setError(''); setSuccess('') try { const session = await supabase.auth.getSession() const token = session.data.session?.access_token const res = await fetch('/api/admin/create-admin', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': Bearer ${token} }, body: JSON.stringify(formAdmin), }) const data = await res.json() if (!res.ok) throw new Error(data.error || 'Erreur création admin') setSuccess(data.message || '✅ Admin créé avec succès') setFormAdmin({ email: '', nom: '' }) loadData() } catch (err: any) { setError(err.message) } finally { setCreating(false) } }
// ── Filtres ────────────────────────────────────────────────────────────────── const partsFiltered = partenaires.filter(p => recherche === '' || p.entreprise.toLowerCase().includes(recherche.toLowerCase()) || p.nom_contact.toLowerCase().includes(recherche.toLowerCase()) || (p.email_pro || '').toLowerCase().includes(recherche.toLowerCase()) )
const coursFiltered = coursiers.filter(c => recherche === '' || (c.nom || '').toLowerCase().includes(recherche.toLowerCase()) || (c.email || '').toLowerCase().includes(recherche.toLowerCase()) )
const clientsFiltered = clients.filter(c => recherche === '' || c.nom.toLowerCase().includes(recherche.toLowerCase()) || c.email.toLowerCase().includes(recherche.toLowerCase()) )
// ── Render ─────────────────────────────────────────────────────────────────── if (loading) return ( 
)
return ( 
{/* ── Navbar ── /} 
NYME ADMIN 
<RefreshCw size={16} className={refreshing ? 'animate-spin' : ''}/> {adminUser?.nom} <button onClick={async () => { await supabase.auth.signOut(); router.replace('/admin-x9k2m/login') }} className="flex items-center gap-1.5 text-sm text-red-300 hover:text-red-100 transition-colors"> Quitter 
{/ Onglets */} 
{ONGLETS.map(o => ( <button key={o.id} onClick={() => { setOnglet(o.id); setRecherche('') }} className={flex items-center gap-1.5 py-3 px-3 text-xs font-semibold border-b-2 transition-all whitespace-nowrap ${ onglet === o.id ? 'border-[#E87722] text-[#E87722]' : 'border-transparent text-white/60 hover:text-white' }}> <o.icon size={14}/> {o.label} {o.id === 'partenaires' && stats.partenaires_attente > 0 && ( {stats.partenaires_attente} )} {o.id === 'coursiers' && stats.coursiers_attente > 0 && ( {stats.coursiers_attente} )} ))} 
 {/* ── Main ── */}
  <main className="max-w-7xl mx-auto p-4 sm:p-6">
    ...
    {onglet === 'tarification' && (
      <TarificationPanel
        baremes={baremes as BaremeAdmin[]}
        configTarif={configTarif as ConfigAdmin | null}
        loadingTarifs={loadingTarifs}
        savingTarif={savingTarif}
        editBareme={editBareme as BaremeAdmin | null}
        setEditBareme={setEditBareme as (b: BaremeAdmin | null) => void}
        saveBareme={saveBareme}
        saveConfig={saveConfig}
        setConfigTarif={setConfigTarif as (c: ConfigAdmin) => void}
